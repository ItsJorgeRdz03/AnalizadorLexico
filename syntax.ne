...
conIgual
...

 {
  lexer: {
    ...
    rules: [
      // Palabras clave
      { regexp: /function/, token: "FUNCTION" },
      { regexp: /if/, token: "IF" },
      { regexp: /else/, token: "ELSE" },
      { regexp: /var/, token: "VAR" },
      { regexp: /let/, token: "LET" },
      { regexp: /const/, token: "CONST" },
      // Identificadores
      { regexp: /[a-zA-Z_][a-zA-Z0-9_]*/, token: "IDENTIFIER" },
      // Números
      { regexp: /^\-?[0-9]*\.?[0-9]+/, token: "NUMBER" },
      // Operadores
      { regexp: /\+|-|\*|\/|%/, token: "OPERATOR" },
      // Otros tokens
      { regexp: /\(|\)/, token: "PAREN" },
      { regexp: /\{|\}/, token: "BRACE" },
      { regexp: /;/, token: "SEMICOLON" },
      { regexp: /\s+/, token: "WHITESPACE" }
    ]
  }
}

%{
  const lexer = require("nearley/lib/nearley-language-bootstrapped");
%}
...
%{
  lexer: {
    ...
    rules: [
      ...
      { regexp: /const/, token: "CONST" },
      // Identificadores
      { regexp: /[a-zA-Z_][a-zA-Z0-9_]*/, token: "IDENTIFICADOR" },
      // Números
      { regexp: /\-?[0-9]*\.?[0-9]+/, token: "NUMERO" },
      { regexp: /\"[^\"]*\"/, token: "STRING" },
      // Operadores
      { regexp: /\+|-|\*|\/|%/, token: "OPERADOR" },
      // Otros tokens
      { regexp: /\(/, token: "APAREN" },
      { regexp: /\)/, token: "CPAREN" },
      { regexp: /\{/, token: "ALLAVE" },
